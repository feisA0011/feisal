...
import { z } from 'zod';
import { addCartLines } from '@/lib/shopify/cart';
import { ShopifyClientError } from '@/lib/shopify/client';
import { checkRateLimit } from '@/lib/security/rate-limit';

const schema = z.object({
  cartId: z.string().min(1),
  lines: z.array(z.object({ merchandiseId: z.string().min(1), quantity: z.number().int().positive() })).min(1)
});

export async function POST(request: Request) {
  try {
    const ip = request.headers.get('x-forwarded-for') ?? 'unknown';
    const rate = checkRateLimit(`cart:add:${ip}`, 40, 60_000);
    if (!rate.allowed) {
    ...
    return NextResponse.json({ error: 'Unexpected error while adding cart lines' }, { status: 500 });
  }
}
