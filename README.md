...
- `/` home
- `/landing` SEO-first campaign/landing route with high-resolution hero imagery
...
## Testimonials (static + server-rendered)

- Testimonials live in `content/testimonials.ts` and are rendered server-side.
- Home renders a lightweight testimonials block.
- PDP filters testimonials by `productHandle` and renders review summary when reviews exist.
- PDP JSON-LD includes `AggregateRating` only if real reviews are available for that product.

## Loading UX without harming CWV

- Route-level skeletons in:
  - `app/products/[handle]/loading.tsx`
  - `app/collections/[handle]/loading.tsx`
  - `app/landing/loading.tsx`
- A tiny top progress indicator component (`components/RouteProgress.tsx`) is used instead of a blocking full-screen spinner.
- Loading UI uses stable containers to avoid layout shifts and does not delay LCP.

## Landing page image quality + performance

- Hero image uses `next/image` with explicit width/height and responsive `sizes`.
- Hero uses `priority` + `placeholder="blur"` (small inline blur data URL).
- Only hero image is preloaded for LCP; below-the-fold images remain lazy.
- AVIF/WebP image output remains enabled via `next.config.ts`.
